...
export function applyCompactionDefaults(cfg: OpenClawConfig): OpenClawConfig {
  ...
  if (compaction.auto === undefined) {
    nextCompaction.auto = true;
    mutated = true;
  }
  if (compaction.maxAutoCompactions === undefined) {
    nextCompaction.maxAutoCompactions = 5;
    mutated = true;
  }
  if (compaction.warnAtCompaction === undefined) {
    nextCompaction.warnAtCompaction = 3;
    mutated = true;
  }
  if (compaction.reserveTokensFloor === undefined) {
    nextCompaction.reserveTokensFloor = 8000;
    mutated = true;
  }
  if (compaction.maxHistoryShare === undefined) {
    nextCompaction.maxHistoryShare = 0.5;
    mutated = true;
  }
  if (!mutated) {
    return cfg;
...
export function applySessionBrainDefaults(cfg: OpenClawConfig): OpenClawConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
  const brain = defaults?.sessionBrain ?? {};
  const nextBrain = { ...brain };
  let mutated = false;

  if (brain.enabled === undefined) {
    nextBrain.enabled = true;
    mutated = true;
  }
  if (brain.maxEntries === undefined) {
    nextBrain.maxEntries = 100;
    mutated = true;
  }
  if (brain.injectOnSessionStart === undefined) {
    nextBrain.injectOnSessionStart = true;
    mutated = true;
  }
  if (brain.updateOnSessionEnd === undefined) {
    nextBrain.updateOnSessionEnd = true;
    mutated = true;
  }
  if (brain.updateOnCompaction === undefined) {
    nextBrain.updateOnCompaction = true;
    mutated = true;
  }
