...
export async function runAgentTurnWithFallback(params: {
  ...
  while (true) {
    try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: (provider, model) => {
          ...
          return runEmbeddedPiAgent({
            ...
            priorCompactionCount: params.getActiveSessionEntry()?.compactionCount ?? 0,
            provider,
            model,
            authProfileId,
      ...
      if (
        embeddedError &&
        embeddedError.kind === "compaction_blocked" &&
        ...
        return {
          ...
          payload: {
            text:
              "⚠️ This session has been compacted many times and automatic compaction is now disabled " +
              "to avoid further quality loss. I've reset our conversation to start fresh — please try again.\n\n" +
              "For long-running tasks, consider exporting important context with `openclaw context export` " +
        ...
        return {
          ...
          payload: {
            text:
              ...
              "To prevent this, increase your compaction buffer (agents.defaults.compaction.reserveTokensFloor) ",
          },
        };
      }
