...
import { shouldBlockFurtherCompaction } from "../compaction-guard.js";
      ...
      try {
        while (true) {
          ...
          if (contextOverflowError) {
            ...
            // Guard: block further auto-compaction when the session has already
            // been compacted maxAutoCompactions times.
            const maxAutoCompactionsRaw =
              params.config?.agents?.defaults?.compaction?.maxAutoCompactions;
            const maxAutoCompactionsLimit =
              typeof maxAutoCompactionsRaw === "number" &&
              Number.isFinite(maxAutoCompactionsRaw) &&
              maxAutoCompactionsRaw > 0
                ? Math.floor(maxAutoCompactionsRaw)
                : 0;
            const lifetimeCompactions = (params.priorCompactionCount ?? 0) + autoCompactionCount;

            if (
              !isCompactionFailure &&
              maxAutoCompactionsLimit > 0 &&
              shouldBlockFurtherCompaction(lifetimeCompactions, {
                maxAutoCompactions: maxAutoCompactionsLimit,
              })
            ) {
              log.warn(
                `[compaction-guard] Blocking auto-compaction for session ${
                  params.sessionKey ?? params.sessionId
                } after ${lifetimeCompactions} compactions (limit=${maxAutoCompactionsLimit}).`,
              );

              const kind = "compaction_blocked" as const;
              const message = `${errorText} (auto-compaction blocked after ${lifetimeCompactions} compactions; limit=${maxAutoCompactionsLimit}).`;

              return {
                payloads: [
                  {
                    text:
                      "Context overflow and automatic compaction has been disabled for this session " +
                      "because it has been compacted many times. " +
                      "To preserve quality, please start a fresh session (for example with /new) " +
                      "or export important context using `openclaw context export`.",
                    isError: true,
                  },
                ],
                meta: {
          ...
          const usagePercent =
            promptTokens && ctxInfo.tokens
              ? Math.min(100, Math.round((promptTokens / ctxInfo.tokens) * 100))
              : 0;
          const toolOutputHeavy =
            attempt.messagesSnapshot && ctxInfo.tokens
              ? sessionLikelyHasOversizedToolResults({
                  messages: attempt.messagesSnapshot,
                  contextWindowTokens: ctxInfo.tokens,
                })
              : false;
          const selectedLayer = selectCompactionLayer({
            usagePercent,
            hasOversizedToolOutput: toolOutputHeavy,
          });
          const resolvedCompaction = resolveCompactionConfig(params.config ?? {});
          if (selectedLayer && params.verboseLevel && params.verboseLevel !== "off") {
            log.info(
              `[compaction-layer] session=${params.sessionKey ?? params.sessionId} layer=${
                selectedLayer.id
              } usage=${usagePercent}%`,
            );
          }

          if (selectedLayer && selectedLayer.id !== "summarize") {
            if (!resolvedCompaction.auto) {
              log.warn(
                `auto-compaction disabled; would have triggered layer=${selectedLayer.id} usage=${usagePercent}% (threshold ${selectedLayer.thresholdPercent}%)`,
              );
            } else {
              log.warn(
                `auto-compaction trigger: layer=${selectedLayer.id} usage=${usagePercent}% (threshold ${selectedLayer.thresholdPercent}%)`,
              );
              const compactResult = await compactEmbeddedPiSessionDirect({
                sessionId: params.sessionId,
                sessionKey: params.sessionKey,
